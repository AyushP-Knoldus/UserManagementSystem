...
class UserRepoUnitTest extends AnyFunSuiteLike {
  ...
  test("Add user to database") {
    ...
    when(mockUserDatabase.add(user)).thenReturn("Added user Ayush to DB.")
  ...
  test("Get a user based on id from database") {
    ...
    when(mockUserDatabase.getById(id)).thenReturn(Some(user))
  ...
  test("Get the list of users from database.") {
    ...
    when(mockUserDatabase.getAll).thenReturn(user)
  ...
  test("Update user based on Id from database.") {
    ...
    when(mockUserDatabase.updateById(id, nameToBeUpdated)).thenReturn(s"Username updated at the id $id")
  ...
  test("Delete a user based on Id from the database.") {
    ...
    when(mockUserDatabase.deleteById(id)).thenReturn(s"User with ID $id deleted from DB.")
  ...
  test("Delete all the users from the database.") {
    when(mockUserDatabase.deleteAll()).thenReturn(ListBuffer.empty[User])
